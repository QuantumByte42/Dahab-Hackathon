  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between gap-2 border-b px-4 bg-gradient-to-r from-amber-50 to-yellow-50 p-6">
      <div className={clsx("flex items-center gap-2", isRTL && "flex-row-reverse")}>
        ...
        <Separator orientation="vertical" className={clsx("h-4", isRTL ? "ml-2" : "mr-2")} />
        <div className={clsx("flex items-center gap-2", isRTL && "flex-row-reverse")}>
          ...
          <div className={clsx(isRTL ? "text-right" : "text-left")}>
            <span className="font-bold text-gray-900">{t("goldStorePOS")}</span>
            <p className="text-xs text-amber-600 font-medium">{t("adminDashboard")}</p>
          </div>
        </div>
      </div>
      <div className={clsx("flex items-center gap-4", isRTL && "flex-row-reverse")}>
        <div className={clsx(isRTL ? "text-left" : "text-right")}>
          ...
          <p className="text-xs text-gray-500">{t("systemAdministrator")}</p>
        </div>
        <LogoutButton />
      </div>
    </header>
